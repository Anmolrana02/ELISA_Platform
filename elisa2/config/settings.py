...
class Settings(BaseSettings):
    ...
    # seq_len=60, patch_size=6 → 10 patches of 6 days (weekly agricultural cycle)
    # d_model=128, n_layers=3 → ~300k params, justified by ~50k clean training rows
    seq_len:          int   = 60
    patch_size:       int   = 6
    d_model:          int   = 128
    n_heads:          int   = 4
...
class CropProfile:
    """
    Holds all FAO-56 agronomy constants for one crop.
    Supports Wheat (MAD-based), Rice (ponding), Sugarcane (MAD-based, deep-root).
    """
    ...
    def __init__(self, name: str, data: dict, soil: dict):
        ...
        if name == "Wheat":
            ...
            # MAD-based like Wheat but with deeper roots and higher MAD
            ...
            # Generic MAD-based fallback for unknown crops
            mad = data.get("mad", 0.50)
            self.trigger_mm    = self.fc_mm - (self.fc_mm - self.pwp_mm) * mad
            self.irr_amount_mm = data.get("irr_amount_mm", 70.0)
    ...
    def kc(self, doy: int) -> float:
        ...
        if self.name == "Sugarcane":
            # Year-round ratoon: initial Jan–Mar, mid Apr–Sep, late Oct–Dec
            if 1   <= doy <= 90:  return tbl["initial"]["value"]   # Jan–Mar  0.40
            if 91  <= doy <= 273: return tbl["mid"]["value"]       # Apr–Sep  1.25
            if 274 <= doy <= 365: return tbl["late"]["value"]      # Oct–Dec  0.75
            return tbl.get("default", 1.00)
...
class AgronomyConfig:
    ...
    def get_crop(self, name: str) -> CropProfile:
        """Returns CropProfile for name; falls back to Wheat for unknowns."""
...
class _AppConfig:
    """Lazy-loaded singleton so YAML is read only once."""
...
if __name__ == "__main__":
    ...
    print(f"Districts  : {list(agro.districts.keys())}")
    print(f"seq_len    : {settings.seq_len}  patch_size: {settings.patch_size}  n_patches: {settings.n_patches}")
    print(f"d_model    : {settings.d_model}  n_layers: {settings.n_layers}")
    for crop_name, crop in agro.crops.items():
        print(f"\n{crop_name}:")
        print(f"  root_depth={crop.root_depth_mm}mm  fc={crop.fc_mm:.1f}mm  "
              f"pwp={crop.pwp_mm:.1f}mm  trigger={crop.trigger_mm:.1f}mm")
        print(f"  irr_amount={crop.irr_amount_mm}mm  growing_months={crop.growing_months}")
    print(f"\nPump kW    : {agro.pump.power_kw:.3f}")
    print(f"Energy/run : {agro.pump.energy_per_run_kwh:.3f} kWh")